...
public class ClientRpcWorker implements Runnable, IObserver {
    ...
    private Response handleLOGIN(Request request) {
        try {
            Optional<Player> opt = server.login((Player) request.data(), this);
    ...
    private Response handleLOGOUT(Request request) {
        try {
            server.logout((Player) request.data(), this);
            connected = false;
    ...
    private Response handleSAVE_GAME(Request request) {
        try {
            server.saveGame((model.Game) request.data());
    ...
    private Response handleGET_CONFIGURATION(Request request) {
        try {
            return new Response.Builder().type(ResponseType.OK).data(server.getConfiguration()).build();
        } catch (Exception e) {
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
        }
    }
