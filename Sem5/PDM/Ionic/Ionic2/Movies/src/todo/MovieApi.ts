...
const getMoviesUrl = `http://${baseUrl}/api/movie`;
const updateMovieUrl = `http://${baseUrl}/api/movie`;
const createMovieUrl = `http://${baseUrl}/api/movie`;

export const getAllMovies: (token: string) => Promise<Movie[]> = (token) => {
    return withLogs(axios.get(getMoviesUrl, authConfig(token)), 'getAllMovies');
}

export const updateMovieAPI: (token: string, movie: Movie) => Promise<Movie[]> = (token, movie) => {
    return withLogs(axios.put(`${updateMovieUrl}/${movie._id}`, movie, authConfig(token)), 'updateMovie');
}
